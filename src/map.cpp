...
bool check_free(int x, int y) {
    return OBSTACLE_MAP[x][y] < (-CONF_SCALE + OBSTACLE_UNKNOWN_BAND); // range from -900 to -1000 ish
}
...
void find_frontier() {
    for (int x = 6; x < MAP_WIDTH - 1 - 4; x++)
    {
        for (int y = 6; y < MAP_HEIGHT - 1 - 4; y++)
        {
            if (x < 15 && y < 15) { // at home
